...
(function () {
    ...
    var broadcastContext = function() {
        const el1 = document.getElementById('txtLoiDanBacSi');
        const el2 = document.getElementById('cboCHANDOANRAVIENID');
        const isVisible = (el) => el && el.offsetWidth > 0 && el.offsetHeight > 0;
        
        if (!isVisible(el1) && !isVisible(el2)) return;
        
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
            chrome.runtime.sendMessage({ type: 'CONTEXT_CHANGED', context: 'DISCHARGE' }).catch(function(){});
        }
    };
