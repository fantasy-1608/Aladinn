/**
 * 🏥 HIS Shared — Crypto Module (SINGLE SOURCE OF TRUTH)
 * PIN hashing, API key encryption/decryption
 * Uses Web Crypto API (PBKDF2 + AES-GCM)
 * 
 * Exported as: HIS.Crypto AND window.Aladinn.Crypto (alias)
 */
...
HIS.Crypto = (function () {
    ...
    /**
     * Verify PIN and cache in session storage for background decryption
     */
    async function verifyAndCachePIN(pin, storedHash, salt) {
    ...
    async function migrateIfNeeded() {
        try {
            ...
            await HIS.Storage.remove(['dashboard_password', 'geminiApiKey']);
    ...
    return {
        generateSalt, hashPIN, verifyPIN, verifyAndCachePIN, deriveKey,
        encrypt, decrypt, encryptAPIKey, decryptAPIKey, migrateIfNeeded
    };
})();

// Alias for Aladinn namespace (backward compat — replaces content/shared/crypto.js)
window.Aladinn = window.Aladinn || {};
window.Aladinn.Crypto = HIS.Crypto;
